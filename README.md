...
The underscore at the end of each observable variable is a SaleCycle convention.  It is not required by Knockout, however by identifying observables with an underscore(‘_’) at the end of a variable name will give our own developers a clear indication that the variable needs to be treated as an observable, not a raw javascript property.

In the first example we see that we are both declaring a property as an observable and also instantiating it with a value. This is not required by knockout, however has became another SaleCycle convention.  By creating an observable with value we clarify that we are setting value to a property.  Similarly we are indicating what type should be stored in an observable. Like plain Javascript the value of an observable isn't type specific. Thus we could call viewModel.house_("dog");  which would have the effect of changing the type of house from an object to a String. Therefore by specifying initial value at declaration we are providing a blueprint of our viewmodels for ourselves and the rest of the team.

###### Round 2: Data-binding

When implementing data-binding in the DOM we are declaring that a property should contain the value (or functionality) of a property within our view model.  We want to do this in a clear, concise and consistent way to provide longevity and maintainability to our code.  Following from our basic example from above, we will extend the view model and demonstrate how SaleCycle will be implementing knockouts data-binding.
...
To see a full version of the validation checkout ko-03

###### Round 4: DOM vs Model manipulation

Knockout provides a rich, yet efficient data-binding tool.  Knockout is non-opinionated, flexible and designed to work well with other tools and libraries.  Owed to these reasons the framework which SaleCycle aims to develop will use Knockout.  However, it is important to prepare for the future by enforcing our own conventions, ensuring our javascript framework will be crafted upon established patterns and principles.  A crucial aim for our framework will be modularity and reusability.  

Using two-way data-binding allows to declare and write our Javascript viewmodel, objects and functions without interfering with the elements they are bound to in the view (The html).  This outlook further decouples the viewmodel from the view as it is not concerned about the actual appearance, but is primarily concerned upon functionality, and properties that enhance the user experience.  To achieve this we have a ‘smarter’ view, as we can see from the comment based syntax, which allows the view to render depending upon our viewmodel.

Model manipulation allows us to be better prepared to enforce modularity.  When we use this outlook, together with a data-injection library such as require.js we can begin to manipulate objects in a manner which emphasises high cohesion and loosely coupled code.  Thus, going forward in establishing the SaleCycle web framework we will conform to primarily use model manipulation to construct, edit and interact with our viewmodels.  This will minimise the risk of bugs appearing from the transition between wireframes and full designs, as the viewmodel should not be concerned with class attributes or html tags.

The diagram below shows how we should aim to build our client side framework, splitting down the project into several components:

![image alt text](image_0.png)

The diagram depicts the the modularity of a framework built upon model manipulation through data-binding and dependency injection.  Our view will simply display the data, the viewmodel will encapsulate data and functionality provided by the modules and services.  Both modules and services will be injected into the viewmodel.  Services will be functions that deal with server interaction only.

*The Final Round*

This document has aimed to achieve a brief description of Knockout.js and the functionality it provides.  Furthermore it has discussed conventions, practices and patterns in which SaleCycle aims to deliver its framework.  The examples provided are both basic, yet sufficient enough replicate how Knockout will be used within the V2 Portal.

This document has not described every single variation of data-binding and other functionality of Knockout.  However it has provided use cases of the most common examples which have been used throughout developing EMR and Workflows.  For reference, the Knockout documentation is very extensive (see [http://knockoutjs.com/](http://knockoutjs.com/) ).
